...
class TripCostListXlsxWriter
{
    ...
    private const HEADERS = [
        'STT', 'Ngày ghi nhận', 'Loại chi phí', 'Số tiền (VND)',
        'Mô tả', 'ID chuyến', 'Trạng thái', 'Người tạo',
    ];
    ...
    public function writeTempFile(array $rows, array $meta = []): string
    {
        ...
        $spreadsheet->getProperties()
            ->setTitle('Danh sách chi phí chuyến')
        ...
        $tmpPath = tempnam(sys_get_temp_dir(), 'trip_costs_export_');
    ...
    private function buildSheet(Worksheet $ws, array $rows, array $meta): void
    {
        ...
        $ws->setCellValue('A1', 'DANH SÁCH CHI PHÍ CHUYẾN');
        ...
        $ws->setCellValue('A3', "Tổng: {$total} dòng chi phí  ·  Bộ lọc: {$filterSummary}");
        ...
        foreach ($rows as $offset => $row) {
            ...
            $values = [
                $offset + 1,
                $this->formatDate($row['reported_on'] ?? null),
                $this->typeLabel($row['type'] ?? null),
                $row['amount'] ?? '',
                $row['description'] ?? '',
                $row['trip_id'] ?? '',

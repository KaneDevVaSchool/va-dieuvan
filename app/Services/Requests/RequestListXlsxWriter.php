...
class RequestListXlsxWriter
{
    ...
    private const HEADERS = [
        'STT', 'Loại chuyến', 'Nơi đi', 'Nơi đến',
        'Ngày giờ đi', 'Ngày giờ về', 'Hành khách',
        'Trạng thái', 'Giá dịch vụ', 'Người yêu cầu', 'Ghi chú',
    ];
    ...
    public function writeTempFile(array $rows, array $meta = []): string
    {
        ...
        $spreadsheet->getProperties()
            ->setTitle('Danh sách phiếu đề xuất')
            ->setCreator('VA Điều Vận')
            ->setCompany('Vietnam America Schools');

        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Phiếu đề xuất');
        $this->buildSheet($sheet, $rows, $meta);

        $tmpPath = tempnam(sys_get_temp_dir(), 'requests_export_');
    ...
    private function buildSheet(Worksheet $ws, array $rows, array $meta): void
    {
        ...
        $ws->setCellValue('A1', 'DANH SÁCH PHIẾU ĐỀ XUẤT ĐIỀU VẬN');
        ...
        $ws->setCellValue('A3', "Tổng: {$total} phiếu  ·  Bộ lọc: {$filterSummary}");
        ...
        foreach ($rows as $offset => $row) {
            ...
            $values = [
                $offset + 1,
                $this->tripTypeLabel($row['trip_type'] ?? null),
                $row['origin'] ?? '',
                $row['destination'] ?? '',
                $this->formatDateTime($row['depart_at'] ?? null),
                $this->formatDateTime($row['arrive_by'] ?? null),
                $row['passenger_count'] ?? '',
                $this->statusLabel($row['status'] ?? null),
                $row['service_price'] ?? '',
                $row['requester_name'] ?? ($row['requester']['name'] ?? ''),
                $row['notes'] ?? '',
            ];
        ...
        $widths = [6, 14, 22, 22, 18, 18, 10, 14, 14, 22, 28];
    ...
    private function tripTypeLabel(mixed $type): string
    {

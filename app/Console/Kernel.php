...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule): void
    {
        ...
        $schedule->command('p2p:generate-trips --for=tomorrow')
            ->dailyAt(config('p2p.generation.primary_at', '22:00'))
            ->withoutOverlapping();
        $schedule->command('p2p:generate-trips --for=today --alert-on-fail')
            ->dailyAt(config('p2p.generation.fallback_at', '05:00'))
            ->withoutOverlapping();

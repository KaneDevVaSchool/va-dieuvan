<template>
  <div class="space-y-4 pb-12">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-slate-900 dark:text-white">{{ t('p2p_policy_page.students_title') }}</h1>
        <p class="text-sm text-slate-500">{{ t('p2p_policy_page.students_subtitle') }}</p>
      </div>
      <div class="flex flex-wrap gap-2">
        ...
        <label class="cursor-pointer rounded-lg bg-va-800 px-3 py-2 text-sm font-medium text-white hover:bg-va-900">
          {{ t('p2p_policy_page.import') }}
          <input type="file" class="sr-only" accept=".xlsx,.xls" @change="onImport" />
        </label>
      </div>
    </div>
...
import {
  importPolicyStudents,
  listAcademicTerms,
  listCampuses,
  listPolicyRoutes,
  listPolicyStudents,
  policyStudentsExportUrl,
} from '../../api/p2pPolicy'
...
const exportUrl = computed(() => {
  const base = policyStudentsExportUrl(apiParams.value)
...
async function onImport(ev) {
  ...
  if (!file || !filters.p2p_policy_term_id) {
    window.alert(t('p2p_policy_page.import_need_term'))
    return
  }
  const fd = new FormData()
  fd.append('file', file)
  fd.append('p2p_policy_term_id', filters.p2p_policy_term_id)
  await importPolicyStudents(fd)
  await reload()
  ev.target.value = ''
}

onMounted(async () => {
  const [r, c, at] = await Promise.all([

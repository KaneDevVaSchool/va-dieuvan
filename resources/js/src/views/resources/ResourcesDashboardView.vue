    ...
    <div class="mx-auto max-w-[1920px] space-y-4 pb-6 text-slate-900">
        ...
        >
            ...
            <div class="flex flex-wrap items-center gap-2">
                <div
                    class="flex items-center gap-1 rounded-lg border border-slate-200 bg-white p-0.5 text-xs shadow-sm"
                >
                    <button
                        ...
                        :title="
                            mode.id !== 'day'
                                ? t('resources_dashboard.view_coming')
                                : ''
                        "
                ...
                <div class="flex flex-wrap items-center gap-1">
                    ...
                    <span
                        class="min-w-[10rem] text-center text-sm font-medium tabular-nums text-slate-900"
                        >{{ dayTitle }}</span
                    >
                    <button
                        type="button"
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-12 xl:items-stretch">
            <div
                class="flex gap-2.5 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:col-span-1 xl:col-span-2"
            >
                <div
                ...
                <div class="min-w-0 flex-1">
                    <div class="text-[11px] font-medium text-slate-500">
                        {{ t("resources_dashboard.kpi_fleet") }}
                    </div>
                    <div
                        v-if="statsLoading"
                        class="mt-1 h-6 w-16 animate-pulse rounded bg-slate-200"
                    />
                    <template v-else>
                        <div
                            class="mt-0.5 text-lg font-bold tabular-nums leading-tight text-slate-900"
                        >
                            {{ vehiclesOperational }} / {{ vehiclesTotal }}
                        </div>
                        <div
                            class="mt-1.5 h-1 overflow-hidden rounded-full bg-slate-100"
                        >
                            <div
            ...
            >
                <div
                    class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-amber-700"
                >
                    <UsersIcon class="h-5 w-5" aria-hidden="true" />
                </div>
                <div class="min-w-0 flex-1">
                    <div class="text-[11px] font-medium text-slate-500">
                        {{ t("resources_dashboard.kpi_drivers") }}
                    </div>
                    <div
                        v-if="statsLoading"
                        class="mt-1 h-6 w-16 animate-pulse rounded bg-slate-200"
                    ...
                    <template v-else>
                        <div
                            class="mt-0.5 text-lg font-bold tabular-nums leading-tight text-slate-900"
                        >
                            {{ driversAvailable }} / {{ driversEmployed }}
                        </div>
                        <div
                            class="mt-1.5 h-1 overflow-hidden rounded-full bg-slate-100"
                        >
            ...
            <div
                class="flex gap-2.5 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:col-span-2 xl:col-span-2"
            >
                <div
                ...
                <div class="min-w-0 flex-1">
                    <div class="text-[11px] font-medium text-slate-500">
                        {{ t("resources_dashboard.kpi_gaps") }}
                    </div>
                    <div
                        v-if="statsLoading"
                        class="mt-1 h-6 w-10 animate-pulse rounded bg-slate-200"
                    />
                    <template v-else>
                        <div class="mt-0.5 flex items-baseline gap-1.5">
                            <span
                                class="text-lg font-bold tabular-nums text-rose-600"
                                >{{ coverageAlertCount }}</span
                            >
            ...
            <div
                class="rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:col-span-2 xl:col-span-6"
            >
                ...
                <div
                    class="mt-2 h-[88px] w-full sm:h-[96px]"
                    role="img"
                    :aria-label="t('resources_dashboard.kpi_forecast')"
                >
                    <svg
                    ...
                    >
                        ...
                        />
                        <polygon
                            :points="forecastDemandFill"
                            fill="url(#rd-fill-dash)"
                        />
                        <polyline
        ...
        >
            ...
            >
                ...
                >
                    <div class="space-y-2 border-b border-slate-200 p-3">
                        <label class="relative block">
                            ...
                            <input
                                ...
                                :placeholder="
                                    t('resources_dashboard.search_placeholder')
                                "
                            />
                        </label>
                        <div
                    ...
                    <div
                        class="min-h-[200px] flex-1 space-y-2 overflow-y-auto p-2 xl:max-h-[calc(100dvh-16rem)]"
                    >
                        <details
                            class="group rounded-lg border border-slate-100 bg-slate-50/50 open:bg-white"
                            open
                        >
                            <summary
                            ...
                            <ul
                                class="space-y-1 border-t border-slate-100 px-1 pb-2 pt-1"
                            >
                                <li
                                    v-for="d in driversFiltered"
                                    :key="'d' + d.id"
                                >
                                    <button
                                        type="button"
                                    ...
                                    >
                                        ...
                                        <span class="min-w-0 flex-1">
                                            ...
                                            <span
                                                class="mt-0.5 flex flex-wrap items-center gap-x-1.5 text-[10px] text-slate-500"
                                            >
                                                <span
                                                    class="rounded bg-teal-50 px-1 py-0.5 font-medium text-teal-800"
                                                    >{{
                                                        tripTypeLabelForDriver(
                                                            d.id,
                                                        )
                                                    }}</span
                                                >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {{
                                                t(
                                                    "resources_dashboard.vehicle_maint_badge",
                                                )
                                            }}
                                        </span>
                                    </button>
                            ...
                            >
                                ...
                                >
                                    <button
                                        ...
                                        class="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-xs transition"
                                        :class="
                                            sidebarRowActive('supplier', p.id)
                                                ? 'bg-teal-50 ring-1 ring-teal-300'
                                                : 'hover:bg-slate-50'
                                        "
                        ...
                        <div class="min-w-[990px] bg-gradient-to-b from-slate-50/40 to-white p-3 dark:from-slate-950/50 dark:to-slate-900/30">
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :style="{
                                        gridTemplateColumns: `repeat(${hourSlots.length}, minmax(0, 1fr))`,
                                    }"
                                >
                                    <div
                            ...
                            <template v-else>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            v-if="row.sub"
                                            class="truncate text-[10px] text-rose-600"
                                            >{{ row.sub }}</span
                                        >
                                        <span
                                            v-else-if="row.meta"
                                ...
                                <p
                                    v-if="!timelineRows.length && !loading"
                ...
                >
                    <div
                        class="border-b border-slate-200/80 bg-white/80 backdrop-blur-sm"
                    >
                        <div
                        ...
                        >
                            ...
                            <button
                                ...
                                :aria-label="
                                    t('resources_dashboard.panel_close')
                                "
                                @click="clearPanel"
                            >
                    ...
                    <!-- Trip -->
                    <div
                        v-if="panel.type === 'trip'"
                        class="flex flex-1 flex-col overflow-hidden"
                    >
                        <div
                            class="flex-1 space-y-4 overflow-y-auto px-3 py-3 text-sm"
                        >
                            <!-- Trip hero card -->
                            <div
                            ...
                            >
                                <div
                                    class="border-l-4 border-teal-500 pl-4 pr-3 pt-3.5 pb-4"
                                >
                                    <div
                                    ...
                                    >
                                        <div class="min-w-0">
                                            ...
                                            >
                                                {{
                                                    t(
                                                        "resources_dashboard.trip_label",
                                                    )
                                                }}
                                            </p>
                                            <h3
                                                class="mt-1 line-clamp-2 text-[15px] font-semibold leading-snug text-slate-900"
                                            >
                                                {{ tripTitle(panel.trip) }}
                                            </h3>
                                            <p
                                                class="mt-1 font-mono text-xs text-teal-700"
                                            >
                                                #{{ panel.trip.id }}
                                            </p>
                                        </div>
                                        <span
                                            ...
                                            :class="
                                                tripStatusBadgeClass(panel.trip)
                                            "
                                        >
                                            {{
                                                labelTripStatus(
                                    ...
                                    <dl
                                        class="mt-4 space-y-2.5 border-t border-slate-100 pt-3 text-xs"
                                    >
                                        <div class="flex justify-between gap-3">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.field_trip_type",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                        ...
                                        <div class="flex justify-between gap-3">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.field_resource",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                        ...
                                        <div class="flex justify-between gap-3">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.field_time",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                                class="text-right tabular-nums font-medium text-slate-900"
                                            >
                                                {{
                                                    fmtTime(
                                                        panel.trip.depart_at,
                                                    )
                                                }}
                                                –
                                                {{
                                                    fmtTime(
                                                        tripEndAt(panel.trip),
                                                    )
                                                }}
                                            </dd>
                                        </div>
                                        <div class="flex justify-between gap-3">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.field_vehicle",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                    ...
                                    <div class="mt-4 grid grid-cols-2 gap-2">
                                        ...
                                        >
                                            ...
                                            {{
                                                t(
                                                    "resources_dashboard.action_edit_trip",
                                                )
                                            }}
                                        </RouterLink>
                                        <RouterLink
                                        ...
                                        >
                                            {{
                                                t(
                                                    "resources_dashboard.action_dispatch_board",
                                                )
                                            }}
                                        </RouterLink>
                                    </div>
                                    ...
                                    >
                                        {{
                                            t(
                                                "resources_dashboard.action_clear",
                                            )
                                        }}
                                    </button>
                                </div>
                            ...
                            <section>
                                ...
                                >
                                    {{
                                        t(
                                            "resources_dashboard.panel_section_compliance",
                                        )
                                    }}
                                </h4>
                                <div
                                    class="rounded-xl border border-slate-200 bg-white p-3 shadow-sm ring-1 ring-slate-900/5"
                                >
                                    <ul class="space-y-2.5 text-[11px]">
                                        <li
                                            class="flex items-start gap-2 text-slate-800"
                                        >
                                            <CheckCircleIcon
                                            ...
                                            <span>{{
                                                t(
                                                    "resources_dashboard.panel_license_ok",
                                                )
                                            }}</span>
                                        </li>
                                        <li
                                        ...
                                        >
                                            ...
                                            <span>{{
                                                t(
                                                    "resources_dashboard.panel_vehicle_ok",
                                                )
                                            }}</span>
                                        </li>
                                    </ul>
                                    ...
                                    >
                                        <div class="flex items-start gap-2">
                                            ...
                                            <div>
                                                <p
                                                    class="text-[11px] font-semibold text-rose-900"
                                                >
                                                    {{
                                                        t(
                                                            "resources_dashboard.shift_limit_title",
                                                        )
                                                    }}
                                                </p>
                                                <p
                                                    class="mt-0.5 text-[11px] leading-snug text-rose-800/95"
                                                >
                                                    {{
                                                        t(
                                                            "resources_dashboard.shift_limit_body",
                                                        )
                                                    }}
                                                </p>
                                            </div>
                                        ...
                                        <div class="flex items-start gap-2">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                >
                                                    {{
                                                        t(
                                                            "resources_dashboard.conflict_block_title",
                                                        )
                                                    }}
                                                </p>
                                                <p
                                                ...
                                                >
                                                    {{
                                                        t(
                                                            "resources_dashboard.panel_trip_conflict",
                                                        )
                                                    }}
                                                </p>
                                                <button
                                                    ...
                                                    @click="
                                                        scrollToSuggestedCoverage
                                                    "
                                                >
                                                    {{
                                                        t(
                                                            "resources_dashboard.action_view_alternatives",
                                                        )
                                                    }}
                                                    →
                                                </button>
                            ...
                            >
                                ...
                                >
                                    {{
                                        t(
                                            "resources_dashboard.panel_section_suggested",
                                        )
                                    }}
                                </h4>
                                <p class="mb-2 text-[10px] text-slate-400">
                                    {{
                                        t(
                                            "resources_dashboard.panel_suggested_context",
                                        )
                                    }}
                                </p>
                                <div
                                ...
                                >
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        >
                                            {{
                                                t(
                                                    "resources_dashboard.est_cost_label",
                                                )
                                            }}:
                                            <span
                                                ...
                                                >{{
                                                    t(
                                                        "resources_dashboard.est_cost_na",
                                                    )
                                                }}</span
                                            >
                                        </p>
                                    ...
                                    <button
                                        ...
                                        :disabled="
                                            assigningNcc || !canAssignTrip
                                        "
                                        @click="assignSuggestedNcc"
                                    >
                                        {{
                                            t(
                                                "resources_dashboard.action_assign_ncc",
                                            )
                                        }}
                                    </button>
                                </div>
                    ...
                    <!-- Driver -->
                    <div
                        v-else-if="panel.type === 'driver'"
                        class="flex flex-1 flex-col overflow-hidden"
                    >
                        <div
                            class="flex-1 space-y-3 overflow-y-auto px-3 py-3 text-sm"
                        >
                            <!-- Compact summary: avatar + label + name + id on one row -->
                            <div
                                class="rounded-xl border border-slate-200/90 bg-white p-3 shadow-sm ring-1 ring-slate-900/5"
                            >
                                <div class="flex items-center gap-3">
                                    <div class="relative shrink-0">
                                        ...
                                        <span
                                            ...
                                            :class="
                                                driverAvailDotClass(panel.data)
                                            "
                                        />
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        ...
                                        >
                                            {{
                                                t(
                                                    "resources_dashboard.panel_driver_section",
                                                )
                                            }}
                                        </p>
                                        <div
                                ...
                                <div class="mt-3 flex gap-2">
                                    ...
                                    >
                                        {{
                                            panelDetailOpen
                                                ? t(
                                                      "resources_dashboard.action_hide_details",
                                                  )
                                                : t(
                                                      "resources_dashboard.action_details",
                                                  )
                                        }}
                                        <ChevronDownIcon
                            ...
                            <div v-show="panelDetailOpen" class="space-y-3">
                                ...
                                >
                                    <div class="flex items-start gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p
                                                class="mt-1 leading-snug opacity-95"
                                            >
                                                {{
                                                    t(
                                                        "resources_dashboard.panel_driver_hours_warn",
                                                    )
                                                }}
                                            </p>
                                        </div>
                                        ...
                                        <dl class="space-y-2.5 text-xs">
                                            ...
                                            >
                                                <dt class="text-slate-500">
                                                    {{
                                                        t(
                                                            "resources_dashboard.panel_driver_phone",
                                                        )
                                                    }}
                                                </dt>
                                                <dd
                                            ...
                                            >
                                                <dt class="text-slate-500">
                                                    {{
                                                        t(
                                                            "resources_dashboard.panel_driver_license",
                                                        )
                                                    }}
                                                </dt>
                                                <dd
                                                    class="text-right tabular-nums text-slate-900"
                                                >
                                                    {{
                                                        panel.data
                                                            .license_expires_at ??
                                            ...
                                            >
                                                <dt class="text-slate-500">
                                                    {{
                                                        t(
                                                            "resources_dashboard.panel_driver_hours",
                                                        )
                                                    }}
                                                </dt>
                                                <dd
                                                    class="text-right font-medium tabular-nums text-slate-900"
                                                >
                                                    {{
                                                        formatHoursShort(
                                                            driverTripHours(
                                            ...
                                            >
                                                <dt class="text-slate-500">
                                                    {{
                                                        t(
                                                            "resources_dashboard.field_status",
                                                        )
                                                    }}
                                                </dt>
                                                <dd
                                ...
                                <div>
                                    ...
                                    >
                                        {{
                                            t(
                                                "resources_dashboard.panel_quick_actions",
                                            )
                                        }}
                                    </p>
                                    <div class="flex flex-col gap-2">
                                        ...
                                        >
                                            ...
                                            {{
                                                t(
                                                    "resources_dashboard.panel_driver_open",
                                                )
                                            }}
                                        </RouterLink>
                                        <RouterLink
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <div
                                    class="border-l-4 border-sky-400 pl-4 pr-3 pt-3.5 pb-4"
                                >
                                    <p
                                        class="text-[10px] font-semibold uppercase tracking-wider text-slate-400"
                                    >
                                        {{
                                            t(
                                                "resources_dashboard.panel_vehicle_section",
                                            )
                                        }}
                                    </p>
                                    <p
                                        class="mt-1 font-mono text-xl font-bold tracking-tight text-slate-900"
                                    >
                                        {{ panel.data.license_plate }}
                                    </p>
                                    <span
                            ...
                            <section>
                                ...
                                >
                                    ...
                                    <dl
                                        class="mt-3 space-y-2.5 border-t border-slate-100 pt-3 text-xs"
                                    >
                                        <div class="flex justify-between gap-2">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.field_type",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                        ...
                                        <div class="flex justify-between gap-2">
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.panel_vehicle_seats",
                                                    )
                                                }}
                                            </dt>
                                            <dd
                                            ...
                                            >
                                                {{
                                                    [
                                                        panel.data.seat_count,
                                                        panel.data.payload_kg,
                                                    ]
                                                        .filter(Boolean)
                                                        .join(" · ") || "—"
                                                }}
                                        ...
                                        <div
                                            v-if="
                                                ...
                                                panel.data.last_maintenance_at
                                            "
                                        ...
                                        >
                                            <dt class="text-slate-500">
                                                {{
                                                    t(
                                                        "resources_dashboard.panel_vehicle_note",
                                                    )
                                                }}
                                            </dt>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                >
                                    {{
                                        t(
                                            "resources_dashboard.panel_vehicle_open",
                                        )
                                    }}
                                </RouterLink>
                                <button
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <div
                                    class="border-l-4 border-emerald-500 pl-4 pr-3 pt-3.5 pb-4"
                                >
                                    <p
                                        class="text-[10px] font-semibold uppercase tracking-wider text-slate-400"
                                    >
                                        {{
                                            t(
                                                "resources_dashboard.panel_supplier_section",
                                            )
                                        }}
                                    </p>
                                    <h3
                                ...
                                <dl class="space-y-2.5 text-xs">
                                    <div class="flex justify-between gap-2">
                                        <dt class="text-slate-500">
                                            {{
                                                t(
                                                    "resources_dashboard.panel_supplier_contact",
                                                )
                                            }}
                                        </dt>
                                        <dd class="text-right text-slate-900">
                                            {{
                                                [
                                                    panel.data.contact_name,
                                                    panel.data.contact_phone,
                                                ]
                                                    .filter(Boolean)
                                                    .join(" · ") || "—"
                                            }}
                                    ...
                                    <div class="flex justify-between gap-2">
                                        <dt class="text-slate-500">
                                            {{
                                                t(
                                                    "resources_dashboard.panel_supplier_contract",
                                                )
                                            }}
                                        </dt>
                                        <dd
...
const dayTitle = computed(() => {
    ...
    if (k === today)
        return `${t("dispatcher_board.today")}, ${shortViDayLabel(k)}`;
    return shortViDayLabel(k);
});

const vehiclesOperational = computed(
    () => vehiclesReady.value + vehiclesInUse.value,
);
...
const driversFiltered = computed(() => {
    ...
    if (filterDriverAvail.value) {
        list = list.filter(
            (d) => d.availability_status === filterDriverAvail.value,
        );
    }
...
const vehiclesFiltered = computed(() => {
    ...
    if (filterVehicleType.value) {
        list = list.filter(
            (v) => (v.type || "").toLowerCase() === filterVehicleType.value,
        );
    }
...
const suppliersFiltered = computed(() => {
    ...
    if (filterSupplier.value === "active")
        list = list.filter((p) => p.is_active);
    else if (filterSupplier.value === "inactive")
        list = list.filter((p) => !p.is_active);
    const q = searchLower.value;
...
const sidebarNavList = computed(() => {
    const out = [];
    for (const d of driversFiltered.value)
        out.push({ kind: "driver", data: d });
    for (const v of vehiclesFiltered.value)
        out.push({ kind: "vehicle", data: v });
    for (const p of suppliersFiltered.value)
        out.push({ kind: "supplier", data: p });
    return out;
});
...
function setPanelDriver(d) {
    ...
    const idx = sidebarNavList.value.findIndex(
        (e) => e.kind === "driver" && e.data.id === d.id,
    );
...
function setPanelVehicle(v) {
    ...
    const idx = sidebarNavList.value.findIndex(
        (e) => e.kind === "vehicle" && e.data.id === v.id,
    );
    sidebarNavIndex.value = idx;
}
...
function setPanelSupplier(p) {
    ...
    const idx = sidebarNavList.value.findIndex(
        (e) => e.kind === "supplier" && e.data.id === p.id,
    );
    sidebarNavIndex.value = idx;
}
...
function computeConflicts(tripList) {
    ...
    for (const group of byDriver.values()) {
        const sorted = [...group].sort(
            (a, b) => new Date(a.depart_at) - new Date(b.depart_at),
        );
...
const timelineTrips = computed(() =>
    trips.value.filter(
        (x) => !["pending", "approved", "cancelled"].includes(x.status),
    ),
);
...
const suggestedProvider = computed(() => {
    ...
    const currentPid =
        trip.transport_provider_id ?? trip.transport_provider?.id ?? null;
...
function tripResourceLabel(trip) {
    const tpId =
        trip.transport_provider_id ??
        trip.transport_provider?.id ??
        trip.transportProvider?.id;
    if (tpId) {
        const name =
            trip.transport_provider?.name ?? trip.transportProvider?.name;
...
function scrollToSuggestedCoverage() {
    document
        .getElementById("panel-suggested-coverage")
...
async function onTripBarPointerUp(e) {
    ...
    try {
        ...
        if (
            t2 &&
            panel.value?.type === "trip" &&
            panel.value.trip.id === trip.id
        ) {
...
async function assignSuggestedNcc() {
    ...
    try {
        ...
        await loadTrips();
        const updated =
            trips.value.find((x) => x.id === trip.id) ??
...
const forecastDemandPoints = computed(() =>
    buildSparklinePoints(tripsPerHour.value, 120, 40, 4),
);
...
async function loadTrips() {
    ...
    try {
        ...
        loadError.value =
            e?.response?.data?.message ?? t("dispatcher_board.load_error");
        trips.value = [];
    } finally {
        loading.value = false;
    }

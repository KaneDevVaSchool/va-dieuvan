  ...
  <div class="space-y-4" data-testid="trip-cost-control-center">
    ...
    <div class="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
      <div
        ...
        :key="kpi.key"
        class="rounded-xl border border-slate-200/80 bg-white p-3.5 shadow-sm dark:border-slate-800 dark:bg-slate-950/40"
      ...
      >
        ...
        <p
          class="mt-1.5 text-base font-bold tabular-nums xl:text-[15px]"
          :class="kpi.valueClass || 'text-slate-900 dark:text-white'"
        >
          {{ kpi.value }}
        </p>
        <p v-if="kpi.sub" class="mt-0.5 text-[11px] font-medium" :class="kpi.subClass || 'text-slate-400'">
          {{ kpi.sub }}
        </p>
      </div>
    </div>
    ...
    <section
      v-if="fin.timeline.value.length"
      class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/40"
      data-testid="cost-timeline"
    >
      <div class="flex items-center justify-between">
        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ t('cost_center.timeline_title') }}</h3>
        <button
          v-if="fin.timeline.value.length > timelineLimit"
          type="button"
          class="text-xs font-semibold text-blue-600 hover:underline"
          @click="timelineExpanded = !timelineExpanded"
        >
          {{ timelineExpanded ? t('cost_center.timeline_collapse') : t('cost_center.timeline_show_all', { n: fin.timeline.value.length }) }}
        </button>
      </div>
      <ol class="mt-3 space-y-2.5">
        <li
          v-for="c in timelineRows"
          :key="c.id"
          class="flex items-center gap-3 text-xs"
        >
          <time class="w-24 shrink-0 tabular-nums text-slate-400">{{ fmtDate(c.created_at) }}</time>
          <span class="h-2 w-2 shrink-0 rounded-sm" :class="groupDot(c.type)" />
          <span class="min-w-0 flex-1 truncate font-medium text-slate-700 dark:text-slate-300">
            {{ groupLabelOf(c.type) }}
            <span v-if="c.description?.trim()" class="font-normal text-slate-400">· {{ c.description.trim() }}</span>
          </span>
          <span class="shrink-0 font-semibold tabular-nums text-slate-800 dark:text-slate-200">{{ fmtMoney(c.amount) }}</span>
          <span class="inline-flex w-16 shrink-0 justify-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold uppercase" :class="statusClass(c.status)">
    ...
    <!-- ─────────── SECTION 5 · DATA GRID ─────────── -->
    <section
      class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/40"
      data-testid="cost-grid"
    >
      <!-- Sticky toolbar -->
      <div
        class="sticky top-0 z-10 flex flex-wrap items-center gap-2 border-b border-slate-100 bg-slate-50/90 px-4 py-2.5 backdrop-blur dark:border-slate-800 dark:bg-slate-900/70"
      >
        <h3 class="mr-auto text-sm font-semibold text-slate-800 dark:text-slate-100">
          {{ t('cost_center.grid_title') }}
          <span class="ml-1 text-xs font-normal text-slate-400">({{ rows.length }})</span>
        </h3>
        ...
        <template v-if="selectedIds.size">
          ...
          <button
            ...
            type="button"
            class="rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-50"
            :disabled="bulkBusy"
          ...
          <button
            v-if="canReconcile && selectedPendingCount"
            type="button"
            class="rounded-lg border border-rose-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-300"
            :disabled="bulkBusy"
        ...
        <button
          type="button"
          class="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
          @click="exportCsv"
        >
          {{ t('cost_center.export_excel') }}
        </button>
        <button
          v-if="canSubmit"
          type="button"
          class="rounded-lg px-2.5 py-1.5 text-xs font-semibold transition"
          :class="showQuickAdd ? 'bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-100' : 'bg-blue-600 text-white hover:bg-blue-700'"
      ...
      <div class="divide-y divide-slate-100 md:hidden dark:divide-slate-800">
        <div v-for="c in rows" :key="c.id" class="px-4 py-3" @click="openDrawer(c)">
        ...
        <p v-if="!rows.length" class="px-4 py-8 text-center text-sm text-slate-400">{{ t('cost_center.empty_title') }}</p>
      </div>
    </section>
...
function fmtMoneySigned(v: number) {
  const sign = v > 0 ? '+' : v < 0 ? '−' : ''
  return `${sign}${new Intl.NumberFormat(locale.value === 'en' ? 'en-US' : 'vi-VN').format(Math.abs(v))} ${currency.value}`
}
...
const kpiCards = computed(() => {
  const cards: { key: string; label: string; value: string; sub?: string; valueClass?: string; subClass?: string }[] = []
  cards.push({ key: 'revenue', label: t('cost_center.kpi_revenue'), value: fmtMoney(fin.revenue.value) })
  cards.push({ key: 'budget', label: t('cost_center.kpi_budget'), value: fmtMoney(fin.budget.value) })
  cards.push({ key: 'actual', label: t('cost_center.kpi_actual'), value: fmtMoney(fin.actualTotal.value) })

  const variance = fin.variance.value
  const vPct = fin.variancePct.value
  cards.push({
    key: 'variance',
    label: t('cost_center.kpi_variance'),
    value: fin.budget.value > 0 ? fmtMoneySigned(variance) : '—',
    valueClass: variance > 0 ? 'text-rose-600' : variance < 0 ? 'text-emerald-600' : 'text-slate-900 dark:text-white',
    sub:
      vPct == null
        ? undefined
        : variance > 0
    ...
    subClass: variance > 0 ? 'text-rose-500' : 'text-emerald-600',
  })

  cards.push({
    key: 'profit',
    label: t('cost_center.kpi_profit'),
    value: fin.revenue.value > 0 ? fmtMoney(fin.profit.value) : '—',
    valueClass: fin.profit.value < 0 ? 'text-rose-600' : 'text-slate-900 dark:text-white',
    sub: fin.profitMargin.value != null ? t('cost_center.margin_pct', { pct: Math.round(fin.profitMargin.value) }) : undefined,
    subClass: fin.profit.value < 0 ? 'text-rose-500' : 'text-emerald-600',
  })

  cards.push({
    key: 'pending',
    label: t('cost_center.kpi_pending'),
    value: t('cost_center.pending_count', { n: fin.pendingCount.value }),
    valueClass: fin.pendingCount.value ? 'text-amber-600' : 'text-slate-900 dark:text-white',
    sub: fin.pendingCount.value ? fmtMoney(fin.pendingTotal.value) : undefined,
...
const selectableIds = computed(() => rows.value.map((c) => c.id).filter((id): id is number => id != null))
...
const timelineRows = computed(() =>
  timelineExpanded.value ? fin.timeline.value : fin.timeline.value.slice(0, timelineLimit),
)

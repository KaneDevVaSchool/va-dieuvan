...
class TripListXlsxWriter
{
    ...
    private const HEADERS = [
        ...
        'Ngày giờ đi', 'Ngày giờ về', 'Biển số xe',
        'Tài xế', 'Trạng thái',
    ];
    ...
    public function writeTempFile(array $rows, array $meta = []): string
    {
        ...
        $sheet->setTitle('Chuyến đi');
        $this->buildSheet($sheet, $rows, $meta);

        $tmpPath = tempnam(sys_get_temp_dir(), 'trips_export_');
    ...
    private function buildSheet(Worksheet $ws, array $rows, array $meta): void
    {
        ...
        $ws->setCellValue('A1', 'DANH SÁCH CHUYẾN ĐI');
        ...
        $ws->setCellValue('A3', "Tổng: {$total} chuyến  ·  Bộ lọc: {$filterSummary}");
        ...
        foreach ($rows as $offset => $row) {
            $r = $dataStart + $offset;
            $dr = $row['dispatchRequest'] ?? $row['dispatch_request'] ?? [];
            $values = [
                $offset + 1,
                $this->tripTypeLabel($dr['trip_type'] ?? null),
                $dr['origin'] ?? '',
                $dr['destination'] ?? '',
        ...
        $widths = [6, 14, 22, 22, 18, 18, 14, 22, 14];

...
const cards = computed(() => {
  ...
  return [
    {
      key: 'total',
      label: t('dept.rejected_kpi_total'),
      tone: 'rose',
      icon: XCircleIcon,
      display: displayCount(total),
      sub: t('dept.rejected_kpi_total_sub'),
      progress: null,
      filter: { kind: 'trip_type', value: '' },
    },
    {
      key: 'month',
      label: t('dept.rejected_kpi_month'),
      tone: 'brand',
      icon: XCircleIcon,
      display: displayCount(s.rejected_this_month ?? 0),
      sub: t('dept.rejected_kpi_month_sub'),
      progress: null,
      filter: { kind: 'month' },
    },
    {
      ...
      tone: 'sky',
      icon: ClockIcon,
      display: displayCount(s.pending_count ?? 0),
      sub: t('dept.rejected_kpi_pending_sub'),
      progress: null,
...
<template>
  <KpiSummaryStrip
    ...
    :aria-label="t('dept.rejected_kpi_strip_aria')"
    :eyebrow="t('dept.approved_kpi_eyebrow')"
    :title="t('dept.rejected_kpi_strip_title')"

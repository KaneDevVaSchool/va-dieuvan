        ...
        <div class="flex w-full min-w-0 flex-wrap items-center gap-2 lg:flex-nowrap">
          <div class="min-w-0 w-full basis-full lg:min-w-[10rem] lg:flex-1 lg:basis-auto">
            <DatagridToolbarSearch
              v-model="searchInput"
              input-id="dept-all-search"
              :placeholder="t('dept.filter_search_placeholder')"
              stretch
              inline-actions
              hide-label
              input-height="h-10"
              data-testid="dept-all-toolbar-search"
              @enter="flushSearch"
            />
          </div>
            ...
            >
              <template #trigger>
                <DatagridToolbarActionButton
                  ...
                  test-id="dept-all-toolbar-filter"
                  @click="openFilterPanel"
                >
                  {{ t('dept.filter_btn') }}
                </DatagridToolbarActionButton>
              </template>
              <li v-for="fd in filterControlDefs" :key="'dept-all-vis-' + fd.key" class="flex items-start gap-2">
                <input
                  :id="`dept-all-filter-vis-${fd.key}`"
                  ...
                  :data-testid="`dept-all-filter-vis-${fd.key}`"
                />
                <label
                  :for="`dept-all-filter-vis-${fd.key}`"
      ...
      >
        <DatagridFilterField v-if="visibleFilters.status">
          <select
            ...
            :aria-label="t('dept.filter_label_status')"
            data-testid="dept-all-filter-status"
            @change="onFilterChange"
          >
            <option v-for="opt in statusFilterOptions" :key="opt.value === '' ? '_all' : opt.value" :value="opt.value">
              {{ opt.label }}
            </option>
          </select>
        </DatagridFilterField>
        ...
        <DatagridFilterField v-if="visibleFilters.trip_type">
          <select
            ...
            data-testid="dept-all-filter-trip-type"
            @change="onFilterChange"
          >
          ...
          <DatagridFilterField>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <FilterDatePicker
                ...
                input-id="dept-all-filter-from"
                data-testid="dept-all-filter-from"
              ...
              <FilterDatePicker
                ...
                input-id="dept-all-filter-to"
                data-testid="dept-all-filter-to"
                @update:model-value="onFilterChange"
              />
            </div>
          </DatagridFilterField>
        ...
        <DatagridFilterField v-if="visibleFilters.per_page">
          <select
            ...
            data-testid="dept-all-filter-per-page"
            @change="onFilterChange"
          >
        ...
        <template v-else>
          ...
          <div class="space-y-4">
            <DeptRequestCard
              ...
              :key="r.id"
              :req="r"
              :show-rejection="r.status === 'rejected'"
              @detail="goDetail"
            />
          </div>
          ...
          <div v-else-if="page < lastPage" class="mt-4 flex justify-center">
            <button
              ...
              data-testid="dept-all-load-more"
              @click="loadMore"
            >
...
import DeptAllSummaryBar from '../../components/dept/DeptAllSummaryBar.vue'
...
const allDatagridRef = ref(null)
useDetailsAutoCloseWithin(allDatagridRef)
...
} = useVisibleFilterControls(ALL_FILTER_CONTROLS, 'va.dept.all.filter_vis_v2')
...
const statusFilterOptions = computed(() => [
  { value: '', label: t('dept.filter_label_status') },
...
async function reloadKpis() {
  ...
  try {
    ...
    const [totalRes, pfRes, apprRes, rejRes, cargoRes] = await Promise.all([
      listRequests(base),
      listRequests({ ...base, status: 'price_filled' }),
      listRequests({ ...base, status: 'approved' }),
      listRequests({ ...base, status: 'rejected' }),
      listRequests({ ...base, trip_type: 'cargo' }),
    ])
    kpiStats.value = {
      total: totalRes.meta?.total ?? 0,
      price_filled: pfRes.meta?.total ?? 0,
      approved: apprRes.meta?.total ?? 0,
      rejected: rejRes.meta?.total ?? 0,

  ...
  >
    ...
    >
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        <h1 class="flex-1 text-center text-xl font-bold tracking-tight">
        ...
        <div class="relative" ref="moreRoot">
          ...
          <div
            v-show="moreOpen"
            class="absolute right-0 top-11 z-20 min-w-[10rem] overflow-hidden rounded-xl border border-white/15 bg-driver-card py-1 text-left text-lg shadow-lg ring-1 ring-white/10"
          >
            <button
              type="button"
      ...
      <template v-if="trip">
        ...
        <div class="mt-3 flex justify-center">
          <span
            class="rounded-full px-4 py-1 text-lg font-semibold"
            :class="statusBadgeClass"
          >{{ headerStatusText }}</span>
        </div>

        <!-- Row 3: date-time -->
        <p class="mt-2 text-center text-lg text-driver-muted/80">{{ formattedDateLine }}</p>
        ...
        <div class="mt-3 grid grid-cols-3 divide-x divide-white/15 rounded-2xl bg-white/10 py-3">
          <div class="flex flex-col items-center gap-0.5 px-2">
            ...
            <span class="text-base text-driver-muted/80">{{ t('driver_trip_detail.stats_students') }}</span>
            <span class="text-xl font-bold tabular-nums text-driver-ink">{{ statsStudentCount }}</span>
          </div>
          <div class="flex flex-col items-center gap-0.5 px-2">
            <MapPinIcon class="h-6 w-6 text-[#7fdcc8]" />
            <span class="text-base text-driver-muted/80">{{ t('driver_trip_detail.stats_distance') }}</span>
            <span class="text-xl font-bold tabular-nums text-driver-ink">{{ statsDistance }}</span>
          </div>
          <div class="flex flex-col items-center gap-0.5 px-2">
            <ClockIcon class="h-6 w-6 text-[#7fdcc8]" />
            <span class="text-base text-driver-muted/80">{{ t('driver_trip_detail.stats_duration') }}</span>
            <span class="text-xl font-bold tabular-nums text-driver-ink">{{ statsDuration }}</span>
          </div>
        </div>
      </template>
      ...
      >
        ...
        <div class="min-w-0">
          <p class="text-lg font-bold">{{ t('driver_trip_detail.warn_title', { n: warningBanner.minutes }) }}</p>
          <p class="mt-0.5 text-base leading-snug text-amber-950/90">{{ warningBanner.body }}</p>
        </div>
      </div>
      </div>
    </div>
    ...
    >
      <p
        v-if="loadError"
        class="mt-3 rounded-xl border border-amber-700/50 bg-amber-950/40 px-3 py-2 text-base text-amber-100 ring-1 ring-amber-600/30"
      >{{ loadError }}</p>
      ...
      <template v-else-if="trip">
        ...
        <div class="overflow-hidden rounded-2xl bg-driver-card ring-1 ring-white/[0.06]">
          <div class="flex items-center gap-2.5 border-b border-white/[0.06] px-4 py-3.5">
            ...
            <span class="flex-1 text-2xl font-bold text-driver-ink">{{ t('driver_trip_detail.route_section') }}</span>
          </div>
          ...
          <div class="px-4 pb-4 pt-2">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1 pt-0.5">
                <p class="text-base font-semibold uppercase tracking-wider text-driver-muted/80">{{ t('driver_trip_detail.point_start') }}</p>
                <p class="text-xl font-bold text-driver-ink">{{ originMain }}</p>
                <p v-if="originSub" class="text-lg text-driver-muted">{{ originSub }}</p>
              </div>
            </div>
            ...
            <div class="mt-3 flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1 pt-0.5">
                <p class="text-base font-semibold uppercase tracking-wider text-driver-muted/80">{{ t('driver_trip_detail.point_end') }}</p>
                <p class="text-xl font-bold text-driver-ink">{{ destMain }}</p>
                <p v-if="destSub" class="text-lg text-driver-muted">{{ destSub }}</p>
              </div>
            </div>
            ...
            <a
              ...
              class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-sky-600 bg-sky-600 py-3.5 text-xl font-bold text-white transition active:opacity-80"
            >
        ...
        <template v-if="paxKind === 'student'">
          <div v-if="paxList.length" class="overflow-hidden rounded-2xl bg-driver-card ring-1 ring-white/[0.06]">
            ...
            <div class="flex items-center justify-between gap-2 px-4 py-3.5">
              <h2 class="flex items-center gap-2 text-lg font-bold text-driver-ink">
              ...
              <div class="flex gap-1.5">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded-full border px-3 py-1.5 text-base font-semibold transition"
                ...
                <button
                  type="button"
                  class="flex items-center gap-1 rounded-full border border-white/10 bg-driver-surface px-3 py-1.5 text-base font-semibold text-driver-muted transition"
                  @click="cycleSort"
                >
              ...
              >
                ...
                >
                  ...
                  <div class="relative shrink-0">
                    <div
                      class="flex h-12 w-12 items-center justify-center rounded-full bg-driver-elevated text-lg font-bold text-driver-muted"
                    >
                      {{ studentInitials(p.name) }}
                    </div>
                    <!-- Number badge -->
                    <div
                      class="absolute -left-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white text-sm font-bold"
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-1.5">
                      <span class="text-lg font-semibold text-driver-ink">{{ p.name }}</span>
                      <span
                        v-if="isNextIndex(p._origIndex) && canMarkPickup"
                        class="rounded-md bg-amber-100 px-1.5 py-0.5 text-sm font-bold uppercase text-amber-700"
                      >{{ t('driver_trip_detail.tag_next') }}</span>
                    </div>
                    <p class="mt-0.5 text-base text-driver-muted">{{ p.subtitle }}</p>
                    <p v-if="p.address" class="mt-0.5 flex items-center gap-1 text-base text-driver-muted/80">
                  ...
                  <div class="flex shrink-0 flex-col items-end gap-1">
                    <span
                      class="rounded-full px-2 py-0.5 text-base font-semibold"
                    ...
                    <span v-if="p.time" class="text-base font-semibold tabular-nums text-driver-muted">{{ p.time }}</span>
                    <ChevronDownIcon
                ...
                >
                  <a
                    v-if="p.phone"
                    :href="`tel:${p.phone}`"
                    class="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500 py-3 text-lg font-bold text-white active:opacity-80"
                  >
                  ...
                  <button
                    ...
                    class="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-[#7fdcc8] py-3 text-lg font-bold text-driver-bg disabled:opacity-50 active:opacity-80"
                  ...
                  <button
                    ...
                    type="button"
                    :disabled="eventPosting"
                    class="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-driver-surface py-3 text-lg font-semibold text-driver-muted disabled:opacity-50 active:opacity-80"
                  ...
                  <button
                    type="button"
                    class="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-driver-surface py-3 text-lg font-semibold text-driver-muted active:bg-driver-elevated"
                  >
          ...
          <p
            v-else
            class="rounded-2xl border border-dashed border-white/10 bg-driver-surface/70 px-3 py-5 text-center text-lg text-driver-muted"
        ...
        <div v-else-if="paxList.length" class="overflow-hidden rounded-2xl bg-driver-card ring-1 ring-white/[0.06]">
          <div class="mb-2 flex items-center justify-between px-4 pt-3.5">
            <h2 class="text-lg font-bold text-driver-ink">
          ...
          <ul class="divide-y divide-white/10 px-4 pb-3">
            ...
            >
              <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-driver-elevated text-base font-bold text-driver-muted">
                {{ studentInitials(p.name) }}
              </div>
              <div class="min-w-0 flex-1">
                <p class="text-lg font-semibold text-driver-ink">{{ p.name }}</p>
                <p class="text-base text-driver-muted">{{ p.subtitle }}</p>
              </div>
              <a
                v-if="p.phone"
                :href="`tel:${p.phone}`"
        ...
        >
          <div class="flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0">
              <p class="text-lg font-semibold text-driver-ink">{{ t('driver_trip_detail.km_row_title') }}</p>
              <p v-if="!hasEndOdometer" class="text-base text-amber-600">{{ t('driver_trip_detail.km_end_missing') }}</p>
              <p v-else class="text-base text-driver-muted">{{ t('driver_trip_detail.km_end_value', { km: formatKm(trip.record.end_odometer_km) }) }}</p>
            </div>
          </div>
        ...
        <div class="overflow-hidden rounded-2xl bg-driver-card ring-1 ring-white/[0.06]">
          <button
            type="button"
            class="flex w-full items-center justify-between gap-2 border-b border-white/[0.06] px-4 py-3.5 text-left active:bg-driver-surface/40"
            :class="canAddCost ? '' : 'cursor-default opacity-95'"
            @click="canAddCost && openCostModal()"
          >
            <div class="flex min-w-0 flex-1 items-center gap-2.5">
              ...
              <span class="text-2xl font-bold text-driver-ink">{{ t('driver_trip_detail.costs_section') }}</span>
              <span v-if="tripCosts.length" class="ml-auto mr-2 shrink-0 truncate text-lg font-semibold text-driver-muted sm:mr-3">{{ formatVnd(costsTotalAmount) }}</span>
            </div>
            <ChevronRightIcon
              class="h-5 w-5 shrink-0 text-driver-muted/60"
              :class="canAddCost ? '' : 'opacity-0'"
            />
          </button>

          <div class="px-4 pb-4 pt-3">
            <div class="mb-3 flex items-center justify-between">
              <span class="text-lg text-driver-muted">
                {{ tripCosts.length ? t('driver_trip_detail.costs_total', { amount: formatVnd(costsTotalAmount) }) : t('driver_trip_detail.costs_empty') }}
              </span>
              <button
                v-if="canAddCost"
                type="button"
                class="flex items-center gap-1 rounded-full bg-driver-bg px-3 py-2 text-lg font-bold text-white active:opacity-80"
                @click="openCostModal"
              >
            ...
            <ul v-if="tripCosts.length" class="space-y-2">
              <li v-for="c in tripCosts" :key="c.id">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-xl font-semibold text-driver-ink">
                      {{ costTypeLabel(c.type) }}
                      <span v-if="c.description" class="font-normal text-driver-muted"> · {{ c.description }}</span>
                    </p>
                    <div class="mt-0.5 flex items-center gap-2">
                      <p class="text-lg text-driver-muted/80">{{ formatCostTime(c.created_at) }}</p>
                      <span
                        class="rounded-full px-1.5 py-0.5 text-base font-semibold"
                  ...
                  <div class="flex shrink-0 items-center gap-0.5 text-xl font-bold tabular-nums text-driver-ink">
                    {{ formatVnd(c.amount) }}
                    <ChevronRightIcon class="h-5 w-5 text-driver-muted/60" />
                  </div>
                </RouterLink>
              </li>
            </ul>
            <p v-else class="py-2 text-center text-lg text-driver-muted/80">{{ t('driver_trip_detail.costs_empty') }}</p>
          </div>
        </div>
    ...
    >
      ...
      >
        <span class="flex-1 text-lg font-semibold text-driver-muted">
        ...
        <button
          type="button"
          class="ml-1 flex items-center gap-1 rounded-full border border-white/10 px-3 py-2 text-base font-bold text-driver-muted transition active:bg-driver-elevated"
      ...
      <button
        ...
        type="button"
        :disabled="actionBusy"
        class="mb-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 py-4 text-lg font-bold text-white shadow-md disabled:opacity-50 active:opacity-80"
        @click="startTrip"
      >
      ...
      <button
        ...
        type="button"
        :disabled="actionBusy"
        class="mb-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-driver-bg py-4 text-lg font-bold text-white shadow-md disabled:opacity-50 active:opacity-80"
        @click="onEndTrip"
      >
        <FlagIcon class="h-5 w-5" />
      ...
      <p
        ...
        class="mb-2 rounded-2xl border border-emerald-500/35 bg-emerald-950/35 py-3.5 text-center text-lg font-medium text-emerald-100 ring-1 ring-emerald-500/25"
      >
    ...
    <Teleport to="body">
      ...
      >
        <div class="w-full max-w-md rounded-t-2xl bg-driver-card p-4 shadow-2xl ring-1 ring-white/10 sm:rounded-2xl" @click.stop>
          <div class="mb-3 flex items-start justify-between gap-2">
            <h3 class="pr-6 text-xl font-bold text-driver-ink">{{ t('driver_trip_detail.km_modal_title') }}</h3>
            <button
              type="button"
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="text-base font-medium text-driver-muted">{{ t('driver_trip_detail.km_start') }}</label>
              <div class="mt-0.5 flex items-center rounded-xl border border-white/10 bg-driver-surface px-2 py-2.5 text-lg text-driver-ink">
                <input class="min-w-0 flex-1 border-0 bg-transparent p-0 text-lg" :value="formatKmInput(startKmModel)" readonly />
                <span class="ml-1 text-base text-driver-muted/80">km</span>
              </div>
            </div>
            <div>
              <label class="text-base font-medium text-driver-muted">{{ t('driver_trip_detail.km_end') }}</label>
              <div class="mt-0.5 flex items-center rounded-xl border border-white/10 bg-driver-surface px-2 py-2.5 focus-within:ring-2 focus-within:ring-[#7fdcc8]/50">
                <input
                  ...
                  type="text"
                  inputmode="numeric"
                  class="min-w-0 flex-1 border-0 bg-transparent p-0 text-lg text-driver-ink placeholder:text-driver-muted/50"
          ...
          <div class="mt-3 flex items-center gap-2 rounded-xl bg-driver-elevated px-3 py-2.5 text-lg text-driver-muted">
          ...
          <div class="mt-3">
            <label class="text-base font-medium text-driver-muted">{{ t('driver_trip_detail.km_note') }}</label>
            <textarea
              v-model="kmNoteModel"
              rows="3"
              class="mt-0.5 w-full resize-none rounded-xl border border-white/10 bg-driver-surface px-3 py-2.5 text-lg text-driver-ink placeholder:text-driver-muted/50 focus:outline-none focus:ring-2 focus:ring-[#7fdcc8]/50"
          ...
          <div class="mt-4 flex gap-2">
            <button
              type="button"
              class="flex-1 rounded-xl border border-white/10 py-3 text-lg font-semibold text-driver-muted"
              @click="kmModalOpen = false"
            >{{ t('driver_trip_detail.cancel') }}</button>
            <button
              type="button"
              :disabled="kmSaving || !canSubmitKm"
              class="inline-flex flex-1 items-center justify-center gap-1 rounded-xl bg-driver-accent py-3 text-lg font-bold text-driver-bg disabled:opacity-50"
              @click="submitKmModal"
            >
      ...
      >
        <div class="w-full max-w-md rounded-t-2xl bg-driver-card p-4 shadow-2xl ring-1 ring-white/10 sm:rounded-2xl" @click.stop>
          <div class="mb-3 flex items-center justify-between">
            <h3 class="text-2xl font-bold text-driver-ink">{{ t('driver_trip_detail.cost_modal_title') }}</h3>
            <button
              type="button"
          ...
          <label class="text-lg font-medium text-driver-muted">{{ t('driver_trip_detail.cost_type') }}</label>
          <div class="relative mt-1.5">
            <select
              v-model="costForm.type"
              class="w-full appearance-none rounded-xl border border-white/10 bg-driver-surface px-3 py-3 pr-11 text-xl text-driver-ink focus:outline-none focus:ring-2 focus:ring-[#7fdcc8]/50"
            >
          ...
          <label class="mt-3 block text-lg font-medium text-driver-muted">{{ t('driver_trip_detail.cost_amount') }}</label>
          <input
            v-model="costForm.amount"
            type="text"
            inputmode="numeric"
            class="mt-0.5 w-full rounded-xl border border-white/10 bg-driver-surface px-3 py-3 text-xl text-driver-ink placeholder:text-driver-muted/50 focus:outline-none focus:ring-2 focus:ring-[#7fdcc8]/50"
            :placeholder="t('driver_trip_detail.cost_amount_ph')"
          />

          <label class="mt-2.5 block text-lg font-medium text-driver-muted">{{ t('driver_trip_detail.cost_desc') }}</label>
          <input
          ...
          <p v-if="costError" class="mt-2 text-lg text-rose-400">{{ costError }}</p>

          <div class="mt-4 flex gap-2">
            <button
              type="button"
              class="flex-1 rounded-xl border border-white/10 py-3 text-xl font-semibold text-driver-muted"
              @click="costModalOpen = false"
            >{{ t('driver_trip_detail.cancel') }}</button>
            <button
              type="button"
              :disabled="costSaving"
              class="flex-1 rounded-xl bg-driver-accent py-3 text-xl font-bold text-driver-bg disabled:opacity-50"
              @click="submitCost"

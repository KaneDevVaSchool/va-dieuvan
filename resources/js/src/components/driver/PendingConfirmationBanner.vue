<template>
  <div class="rounded-3xl border border-slate-700/70 bg-[#111827] shadow-xl shadow-black/25 ring-1 ring-white/5">
    <!-- Header -->
    <div class="border-b border-slate-700/60 px-4 py-4 sm:px-5">
      <div class="flex items-start gap-3">
        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-amber-500/15 ring-1 ring-amber-500/25">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-6 w-6 text-amber-400" aria-hidden="true">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p v-if="!loading" class="mt-1 text-sm text-slate-400">
    ...
    <div v-if="loading" class="space-y-4 px-4 py-4 sm:px-5">
      <p class="text-sm text-slate-500">{{ t('driver_home.pending_loading_hint') }}</p>
      <div v-for="s in 3" :key="s" class="rounded-2xl border border-slate-700/50 bg-[#1E293B]/80 p-4">
        <div class="h-5 w-40 animate-pulse rounded bg-slate-600/50" />
        <div class="mt-4 space-y-3">
          <div class="h-24 animate-pulse rounded-xl bg-slate-600/40" />
          <div class="h-24 animate-pulse rounded-xl bg-slate-600/40" />
        </div>
      </div>
    </div>

...
export function buildBm03BodyFromWizardSnapshot(snapshot) {
  ...
  const f = snapshot.form && typeof snapshot.form === 'object' ? snapshot.form : {}
  const passengerRows = Array.isArray(snapshot.passengerRows) ? snapshot.passengerRows : []
  const businessRows = Array.isArray(snapshot.businessRows) ? snapshot.businessRows : []
  const cargoRows = Array.isArray(snapshot.cargoRows) ? snapshot.cargoRows : []

<template>
  ...
  <div v-if="loading" class="space-y-3">
    <div class="animate-pulse rounded-2xl bg-slate-200 dark:bg-slate-800 h-52" />
  </div>
  ...
  >
    ...
    <div class="flex items-center justify-between gap-2 px-4 pt-4">
      ...
      <span class="shrink-0 text-base tabular-nums font-semibold text-white/80">
        {{ timeRange }}
      </span>
    </div>
    ...
    <div class="mt-3 space-y-2 px-4">
      ...
      <div class="flex items-start gap-3">
        <span class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-500/20">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4 text-emerald-400">
        ...
        <div class="min-w-0">
          <p class="text-[11px] font-medium uppercase text-white/50">Điểm đón</p>
          <p class="truncate text-base font-semibold leading-snug">{{ origin }}</p>
        </div>
      </div>

      <!-- Dropoff -->
      <div class="flex items-start gap-3">
        <span class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-sky-500/20">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4 text-sky-400">
            <path fill-rule="evenodd" d="M10 1a.75.75 0 01.75.75v1.5a.75.75 0 01-1.5 0v-1.5A.75.75 0 0110 1zM5.05 3.05a.75.75 0 011.06 0l1.062 1.06A.75.75 0 116.11 5.173L5.05 4.11a.75.75 0 010-1.06zm9.9 0a.75.75 0 010 1.06l-1.06 1.062a.75.75 0 01-1.062-1.061l1.061-1.061a.75.75 0 011.06 0zM10 8a2 2 0 100 4 2 2 0 000-4zm-7.75 2a.75.75 0 000 1.5h1.5a.75.75 0 000-1.5h-1.5zm14.5 0a.75.75 0 000 1.5h1.5a.75.75 0 000-1.5h-1.5zM5.05 16.95a.75.75 0 001.06-1.06l-1.06-1.062a.75.75 0 00-1.061 1.06l1.06 1.062zm9.9-1.06a.75.75 0 00-1.061-1.061l-1.062 1.06a.75.75 0 001.06 1.062l1.062-1.061zM10 16a.75.75 0 01.75.75v1.5a.75.75 0 01-1.5 0v-1.5A.75.75 0 0110 16z" clip-rule="evenodd" />
          </svg>
        </span>
        <div class="min-w-0">
          <p class="text-[11px] font-medium uppercase text-white/50">Điểm đến</p>
          <p class="truncate text-base font-semibold leading-snug">{{ destination }}</p>
        </div>
      </div>
...
const ctaClass = computed(() => {
  if (status.value === 'in_progress') return 'bg-amber-500 text-white hover:bg-amber-400'

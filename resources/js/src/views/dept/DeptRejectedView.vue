              ...
              <div class="p-3 pt-2">
                ...
                <div class="mt-3 border-t border-slate-100 pt-3 dark:border-slate-700">
                  ...
                  <ul class="mt-2 max-h-[min(40vh,220px)] space-y-2 overflow-y-auto pr-0.5">
                    <li v-for="fd in filterControlDefs" :key="'dept-rejected-vis-' + fd.id" class="flex items-start gap-2">
                      <input
                        :id="'dept-rejected-filter-vis-' + fd.id"
                      ...
                      />
                      <label
                        :for="'dept-rejected-filter-vis-' + fd.id"
          ...
          <div class="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
            ...
            >
              <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <label class="sr-only" for="dept-rejected-date-from">{{ t('dept.filter_date_from') }}</label>
                <input
                  id="dept-rejected-date-from"
                ...
                <label class="sr-only" for="dept-rejected-date-to">{{ t('dept.filter_date_to') }}</label>
                <input
                  id="dept-rejected-date-to"
                  v-model="filters.to"
                  type="date"
                  class="h-9 w-full rounded-md border-0 bg-white px-2 text-sm text-slate-900 shadow-sm ring-1 ring-slate-200/80 focus:outline-none focus:ring-2 focus:ring-teal-500/30 sm:w-auto dark:bg-slate-950 dark:text-slate-100 dark:ring-slate-600"
                  @change="onDateFilterChange"

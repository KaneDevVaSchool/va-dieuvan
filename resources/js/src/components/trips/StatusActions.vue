  ...
  >
    ...
    <div class="mt-2 flex flex-wrap items-center gap-2">
      <button
        ...
        @click="$emit('assign')"
      >
        <span v-if="assigning" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-white/50 border-t-white" aria-hidden="true"></span>
        {{ t('trip_detail.status_workflow.assign_vehicle') }}
      </button>

      <button
        v-else-if="workflow.primaryAction?.kind === 'advance' && workflow.primaryAction.to === 'in_progress'"
        type="button"
        :disabled="statusBusy"
        class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
        @click="$emit('advance', 'in_progress')"
      >
        <span v-if="statusing" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-white/50 border-t-white" aria-hidden="true"></span>
        {{ t('trip_detail.status_workflow.start_trip') }}
      </button>

      <button
        v-else-if="workflow.primaryAction?.kind === 'advance' && workflow.primaryAction.to === 'completed'"
        type="button"
        :disabled="statusBusy"
        class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <template v-if="workflow.isTerminal">
        <p class="w-full text-sm text-slate-500 dark:text-slate-400">{{ workflow.hintI18nKey ? t(workflow.hintI18nKey) : '' }}</p>
      </template>
      ...
      <button
        ...
        @click="$emit('advance', 'in_progress')"
      >
        <span v-if="statusing" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-slate-400 border-t-slate-800" aria-hidden="true"></span>
      ...
      <button
        ...
        @click="$emit('cancel')"
      >
        <span v-if="rejecting" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-rose-300 border-t-rose-600" aria-hidden="true"></span>
...
import { resolveTripStatusWorkflow, tripStatusBadgeClass } from '../../composables/useTripStatusWorkflow'
...
const props = defineProps<{
  ...
  assigning: boolean
  rejecting: boolean
  /** True while updateTripStatus in flight */
  statusing: boolean
  modelValue: string
}>()

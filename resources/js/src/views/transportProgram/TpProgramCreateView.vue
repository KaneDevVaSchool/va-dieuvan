<template>
  <div class="pb-28">
    ...
    <div class="sticky top-0 z-30 -mx-4 mb-5 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur md:-mx-6 md:px-6">
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex shrink-0 items-center gap-2">
          <button
            type="button"
            class="relative grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50"
            aria-label="Thông báo"
            @click="goNotifications"
          >
            <BellIcon class="h-5 w-5" />
            <span class="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-rose-500"></span>
          </button>
          <button
            type="button"
    ...
    <div class="mx-auto max-w-3xl space-y-5">
      ...
      <div class="sticky top-[57px] z-20 -mx-1 overflow-x-auto px-1 pb-1">
      ...
      <!-- 1. Thông tin cơ bản -->
      <section :ref="setSectionRef('basic')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500">Tên chương trình, năm học và mô tả</p>
          </div>
        </header>
        <div class="space-y-4 p-5">
          <div>
            ...
            <input
              ...
              :class="[
                'w-full rounded-lg border bg-white px-3 py-2.5 text-sm outline-none transition focus:ring',
                errors.name
              ...
              @input="errors.name = ''"
            />
            <p v-if="errors.name" class="mt-1 flex items-center gap-1 text-xs text-rose-600">
              <ExclamationCircleIcon class="h-3.5 w-3.5" /> {{ errors.name }}
            </p>
          </div>
      ...
      <!-- 2. Tuyến đường -->
      <section :ref="setSectionRef('route')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-emerald-50 text-emerald-600">
            <MapIcon class="h-5 w-5" />
          </span>
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Tuyến đường</h2>
            <p class="text-xs text-slate-500">Điểm xuất phát, điểm đến và các điểm dừng</p>
          </div>
        </header>
        <div class="space-y-4 p-5">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <div class="relative">
                <MapPinIcon class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-500" />
                <input
            ...
            <div>
              ...
              <div class="relative">
                <MapPinIcon class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-rose-500" />
                <input
          ...
          <div>
            <div class="mb-1.5 flex items-center justify-between">
              <span class="text-xs font-medium text-slate-600">Các điểm dừng trung gian</span>
              <button
                type="button"
                class="inline-flex items-center gap-1 text-xs font-medium text-va-800 transition hover:underline"
                @click="addStop"
              >
                <PlusIcon class="h-3.5 w-3.5" /> Thêm điểm dừng
              </button>
            </div>
            <div v-if="!form.stops.length" class="rounded-lg border border-dashed border-slate-200 py-4 text-center text-xs text-slate-400">
              Chưa có điểm dừng — nhấn "Thêm điểm dừng" để bổ sung.
            </div>
            <div v-else class="space-y-2">
              ...
              >
                <span class="grid h-2.5 w-2.5 shrink-0 place-items-center rounded-full bg-slate-300"></span>
                <input
                  v-model.trim="stop.name"
                  type="text"
                  :placeholder="`Điểm dừng ${i + 1}`"
                  class="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring"
                />
                <div class="relative w-28 shrink-0">
                  <input
                    v-model="stop.time"
                    type="time"
                    class="w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring"
                  />
                </div>
                <button
                  type="button"
                  class="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-slate-400 transition hover:bg-rose-50 hover:text-rose-500"
      ...
      <!-- 3. Lịch trình -->
      <section :ref="setSectionRef('schedule')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-violet-50 text-violet-600">
            <CalendarDaysIcon class="h-5 w-5" />
          </span>
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Lịch trình</h2>
            <p class="text-xs text-slate-500">Ngày hoạt động, giờ đi và giờ về</p>
          </div>
        </header>
        <div class="space-y-4 p-5">
          <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1.5">
              <button
                ...
                type="button"
                :class="[
                  'h-9 w-10 rounded-lg border text-xs font-semibold transition',
            ...
            <p v-if="errors.runs_on" class="mt-1 text-xs text-rose-600">{{ errors.runs_on }}</p>
          </div>
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-slate-800">Chuyến Sáng (Đưa đến trường)</span>
              </div>
              <button
                type="button"
                role="switch"
            ...
            <div v-if="form.morning_enabled" class="mt-3 grid gap-4 sm:grid-cols-2">
              <div>
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-slate-800">Chuyến Chiều (Đón về nhà)</span>
              </div>
              <button
                type="button"
                role="switch"
            ...
            <div v-if="form.afternoon_enabled" class="mt-3 grid gap-4 sm:grid-cols-2">
              <div>
              ...
              <div>
                <label class="mb-1 block text-xs font-medium text-slate-600">Giờ về đến khu vực</label>
                <input v-model="form.afternoon_arrival" type="time" :class="timeClass" />
              </div>
            </div>
          </div>
          <p v-if="errors.trips" class="text-xs text-rose-600">{{ errors.trips }}</p>
        </div>
      </section>

      <!-- 4. Sức chứa & Xe -->
      <section :ref="setSectionRef('capacity')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-teal-50 text-teal-600">
            <UserGroupIcon class="h-5 w-5" />
          </span>
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Sức chứa &amp; Xe</h2>
            <p class="text-xs text-slate-500">Thông tin xe và số lượng học sinh tối đa</p>
          </div>
        </header>
        <div class="space-y-4 p-5">
          <div class="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <select v-model="form.vehicle_type" :class="selectClass">
                <option value="">— Chọn loại xe —</option>
                <option v-for="t in vehicleTypeOptions" :key="t" :value="t">{{ t }}</option>
              </select>
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-slate-600">Biển số xe</label>
              <input
                v-model.trim="form.plate_number"
                type="text"
                placeholder="30A-56789"
                class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring"
              />
            </div>
            <div>
              <label class="mb-1 block text-xs font-medium text-slate-600">
              ...
              <div class="relative">
                <input
                  ...
                  type="number"
                  min="1"
                  class="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-3 pr-16 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring"
                />
                <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">học sinh</span>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-xl border border-sky-200 bg-sky-50/60 p-4">
            <label class="flex items-start gap-2.5">
              <input
                v-model="form.registration_limit_enabled"
                type="checkbox"
                class="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-va-800 focus:ring-va-800/30"
              />
              <span>
                <span class="block text-sm font-semibold text-slate-800">Giới hạn đăng ký</span>
                <span class="block text-xs text-slate-500">Tự động đóng đăng ký khi đạt giới hạn chỗ ngồi</span>
              </span>
            </label>
            <div v-if="form.registration_limit_enabled" class="mt-3 grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1 block text-xs font-medium text-slate-600">Số chỗ đăng ký tối đa</label>
                <input
              ...
              <div>
                <label class="mb-1 block text-xs font-medium text-slate-600">Số chỗ dự phòng</label>
                <input
                  v-model.number="form.reserve_seats"
                  type="number"
                  min="0"
                  class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring"
                />
          ...
          <div>
            <div class="mb-1 flex items-center justify-between text-xs">
              <span class="font-medium text-slate-600">Tỉ lệ sử dụng xe</span>
              <span class="font-semibold text-slate-700">
                {{ usedSeats }}/{{ form.max_capacity || 0 }} chỗ
                <span class="ml-1 text-emerald-600">{{ usagePct }}%</span>
              </span>
            </div>
      ...
      <!-- 5. Tài xế phụ trách -->
      <section :ref="setSectionRef('drivers')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-amber-50 text-amber-600">
            <IdentificationIcon class="h-5 w-5" />
          </span>
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Tài xế phụ trách</h2>
            <p class="text-xs text-slate-500">Phân công tài xế và người phụ trách chương trình</p>
          </div>
        </header>
        <div class="space-y-4 p-5">
          ...
          <template v-else>
            ...
            <div>
              ...
              <div v-else class="rounded-xl border border-dashed border-slate-200 px-4 py-3 text-xs text-slate-400">
              ...
              <p v-if="errors.driver" class="mt-1 text-xs text-rose-600">{{ errors.driver }}</p>
              ...
              <div v-if="changingMain" class="mt-2 max-h-56 space-y-1 overflow-y-auto rounded-xl border border-slate-200 p-1">
                ...
                >
                  ...
                  <span class="min-w-0 flex-1">
                    ...
                    <span class="block truncate text-[11px] text-slate-400">GPLX: {{ d.license_class || '—' }}</span>
                  </span>
                  <span :class="availabilityBadgeClass(d.availability_status)">{{ availabilityLabel(d.availability_status) }}</span>
                </button>
                <p v-if="!drivers.length" class="px-3 py-2 text-xs text-slate-400">Không có tài xế khả dụng.</p>
              </div>
            </div>

            <!-- Tài xế phụ -->
            <div>
              <label class="mb-1.5 block text-xs font-medium text-slate-600">Tài xế phụ / dự phòng</label>
              <div class="divide-y divide-slate-100 rounded-xl border border-slate-200">
                ...
                <p v-if="!secondaryCandidates.length" class="px-4 py-3 text-xs text-slate-400">Không còn tài xế nào khác.</p>
              </div>
            </div>

            <!-- Người phụ trách -->
            <div>
              <label class="mb-1 block text-xs font-medium text-slate-600">Người phụ trách / Giám sát</label>
      ...
      <!-- 6. Ghi chú -->
      <section :ref="setSectionRef('notes')" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-500">
            <DocumentTextIcon class="h-5 w-5" />
          </span>
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Ghi chú</h2>
            <p class="text-xs text-slate-500">Lưu ý nội bộ cho chương trình (tùy chọn)</p>
          </div>
        </header>
        <div class="p-5">
          <textarea
    ...
    <div class="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur md:px-6">
...
const selectClass =
  'w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring'
const textareaClass =
  'w-full resize-y rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring'
const timeClass =
  'w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none ring-va-800/20 transition focus:border-va-800/40 focus:ring'
...
const usedSeats = computed(() => {
  if (form.registration_limit_enabled) return Number(form.max_registrations || 0)
  return 0
})
const usagePct = computed(() => {
  const cap = Number(form.max_capacity || 0)
  if (!cap) return 0
  return Math.min(100, Math.round((usedSeats.value / cap) * 100))
})

// ── Stops ────────────────────────────────────────────────────────────────────
function addStop() {
  form.stops.push({ uid: ++stopUid, name: '', time: '' })
}
...
function buildPayload() {
  ...
  return {
    ...
    settings: {
      ...
      stops: form.stops
        .filter((s) => s.name)
        .map((s) => ({ name: s.name, time: s.time || null })),
      morning: {
      ...
      registration: {
        limit_enabled: form.registration_limit_enabled,
        max_registrations: Number(form.max_registrations || 0),
        reserve_seats: Number(form.reserve_seats || 0),
      },

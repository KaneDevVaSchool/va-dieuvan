<template>
  ...
  >
          ...
          <div v-if="canExport" ref="exportMenuRef" class="relative">
            ...
            >
              <p class="px-3 pb-1 pt-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                {{ t('driver_freq.export_group_filtered') }}
              </p>
              <button
                type="button"
              ...
              <p class="px-3 pb-1 pt-0.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                {{ t('driver_freq.export_group_all') }}
              </p>
              <button
                type="button"
                class="flex w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
                data-testid="driver-freq-filters-export-xlsx-all"
                :disabled="!!exporting || loading"
                @click="onExportXlsxAll"
              >
                {{ t('driver_freq.btn_export_xlsx_all') }}
              </button>
              <button
                type="button"
                class="flex w-full px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
                data-testid="driver-freq-filters-export-pdf-all"
                :disabled="!!exporting || loading"
                @click="onExportPdfAll"
              >
                {{ t('driver_freq.btn_export_pdf_all') }}
              </button>
            </div>
          </div>
        </div>
    ...
    >
      ...
      <DatagridFilterField v-if="filterControlVisible.month">
        <select
          :value="filters.month"
          :class="FILTER_CONTROL_CLASS"
          :aria-label="t('driver_freq.filter_vis_month')"
          data-testid="driver-freq-filter-month"
          @change="$emit('patch-filter', { month: $event.target.value })"
        >
          <option value="">{{ t('driver_freq.filter_month') }}</option>

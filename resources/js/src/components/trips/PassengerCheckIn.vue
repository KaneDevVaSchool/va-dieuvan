  ...
  <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm ring-1 ring-slate-100/90 dark:border-slate-700/80 dark:bg-slate-950/25 dark:ring-slate-700/50">
    <div class="flex flex-col gap-3 border-b border-slate-100 pb-3 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex flex-wrap items-center gap-2">
        ...
        >
          {{ t('trip_detail.passengers.checkin_header_badge', { checked: checkedCount, total: rows.length }) }}
        </span>
      </div>
      ...
      <template v-if="!editMode">
        <table v-if="rows.length" class="min-w-full divide-y divide-slate-100 text-sm">
          <thead class="bg-slate-50/80">
            <tr>
              <th v-if="canCheckIn" class="w-10 px-2 py-2.5 text-left text-xs font-semibold text-slate-600" />
          ...
          <tbody class="divide-y divide-slate-100 bg-white">
            <template v-for="row in rows" :key="row.passengerKey">
              <tr
                ...
                :class="[
                  rowExpanded === row.passengerKey ? 'bg-slate-50/80' : '',
                  checkedLocal[row.passengerKey] ? 'bg-emerald-50/90' : '',
                ]"
                @click.self="toggleExpand(row.passengerKey)"
              >
                <td v-if="canCheckIn" class="px-2 py-2.5 align-middle" @click.stop>
                  <input
                    type="checkbox"
                ...
                <td class="px-3 py-2.5 align-top" @click="toggleExpand(row.passengerKey)">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div v-if="checkedLocal[row.passengerKey]" class="mt-0.5 text-[10px] text-emerald-700">
                        {{ checkTimeLabel(row.passengerKey) }}
                      </div>
                    </div>
                  </div>
                </td>
                <td class="px-3 py-2.5 align-top" @click="toggleExpand(row.passengerKey)">
                  ...
                  >
                    {{ t('trip_detail.passengers.checkin_on_vehicle') }}
                  </span>
                  <span
                    v-else-if="canCheckIn"
                    class="ml-1 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600"
                  >
                    {{ t('trip_detail.passengers.checkin_waiting') }}
                  </span>
                </td>
                <td class="max-w-[160px] px-3 py-2.5 align-top" @click.stop>
                  <template v-if="telHref(row.contact)">
                    <a :href="telHref(row.contact) || undefined" class="text-sky-700 underline-offset-2 hover:underline">
                      {{ row.contact }}
                    </a>
                  </template>
                  <span v-else class="text-slate-600">{{ row.contact || '—' }}</span>
                </td>
              ...
              <tr v-if="rowExpanded === row.passengerKey" class="bg-slate-50/60">
                <td :colspan="canCheckIn ? 5 : 4" class="px-4 py-3 text-xs text-slate-700">
                  <div class="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div class="font-semibold text-slate-500">{{ t('trip_detail.passengers.checkin_expand_pickup') }}</div>
                      <div class="mt-0.5">{{ row.pickupAddress?.trim() || '—' }}</div>
                    </div>
                    <div>
                      <div class="font-semibold text-slate-500">{{ t('trip_detail.passengers.checkin_private_note') }}</div>
                      <div class="mt-0.5 whitespace-pre-wrap">{{ row.notes?.trim() || '—' }}</div>
                    </div>
                  </div>
                </td>
              </tr>
        ...
        <div v-if="!rows.length" class="px-3 py-6 text-center text-sm text-slate-500">
...
function syncFromTrip() {
  const m = props.trip?.passenger_check_ins as Record<string, { checked_in_at?: string }> | undefined
...
const checkedCount = computed(() => props.rows.filter((r) => checkedLocal.value[r.passengerKey]).length)

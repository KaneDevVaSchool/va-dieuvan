    ...
    <template v-else>
      ...
      <div class="mt-2.5 flex flex-wrap items-center gap-2">
        <button
          ...
          @click="$emit('assign')"
        >
          <span v-if="assigning" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-white/50 border-t-white" aria-hidden="true" />
        ...
        >
          <span v-if="statusing" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-white/50 border-t-white" aria-hidden="true" />
        ...
        <button
          ...
          @click="$emit('advance', 'in_progress')"
        >
          <span v-if="statusing" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-slate-400 border-t-slate-800" aria-hidden="true" />
        ...
        <button
          ...
          @click="$emit('cancel')"
        >
          <span v-if="rejecting" class="mr-2 inline-block h-3 w-3 animate-spin rounded-full border border-rose-300 border-t-rose-600" aria-hidden="true" />
          {{ t('trip_detail.status_workflow.cancel_outlined') }}
        </button>
      </div>

      <!-- Hint -->
      <p v-if="workflow.hintI18nKey" class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
